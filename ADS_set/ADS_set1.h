...
template <typename Key, size_t N>
typename ADS_set<Key, N>::Iterator ADS_set<Key, N>::y()const 
{
   if (this->size() <= 3) 
   {
        // Если элементов меньше 3, возвращаем итератор в обычном режиме для всех элементов.
        // "Специальный" режим здесь не требуется.
        for (size_t i = 0; i < directory_size; ++i) {
            if (buckets[i]->get_size() > 0) {
                // Используем базовый конструктор итератора для начала итерации
                return Iterator(this, i, 0,buckets[i]->get_value(0),buckets[i]->get_value(1),true);
            }
        }
   }
   ...
   else
   {
        for (size_t i = 0; i < directory_size; ++i) 
        {
            if(buckets[i]->get_size() > 0) 
            {
               return iterator(this, i, 0,buckets[i]->get_value(0),buckets[i]->get_value(1),false); 
            }
        } 
   }
...
class ADS_set<Key,N>::Iterator 
{
   ...
   explicit Iterator(const ADS_set* s, size_type bucket_index, size_type elem_index,value_type first_element,value_type second_element,bool mode): s{s},bucket_index{bucket_index},elem_index{elem_index},first_element{first_element},second_element{second_element},mode{mode}{}
   ...
   Iterator& operator++()
   {
      increment_helper(); // Переходим к следующему элементу
      if (*this == this->s->end()) // Если достигнут конец контейнера
         return *this;

      if (mode) 
      {  
         // Если находимся в режиме "нормальном"
         return *this; // Поведение остается без изменений
      }
         /*
         value_type value1 = first_element; // Первый элемент
         value_type value2 = second_element;// Второй элемент
         value_type current_element = **this;
         if ((std::less<value_type>{}(value2,value1) && !std::less<value_type>{}(current_element,value2)) || (!std::less<value_type>{}(value2,value1) && std::less<value_type>{}(current_element,value2))) {
        return ++(*this); // Рекурсивно пропускаем неподходящий элемент
    }

      // Если текущий элемент подходит, обновляем second_element и возвращаем итератор
      value2 = current_element;
      */
      return *this;
